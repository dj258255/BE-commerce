...
**평가 예시**: mode 의 요청 시각 하나. 대상 고객 = 그 주에 산 적이 있고 이력에 어휘 상품이 하나 이상 있는 고객(v1 의 "평가 고객 68,984" 와 같은 정의).
...
- 상품 자리: 그 행 소속만(`ROW_REPEAT` 는 `history_articles`), 페이지 안 중복 · `exclude_items` 금지
