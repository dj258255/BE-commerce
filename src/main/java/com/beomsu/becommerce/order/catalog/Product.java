 ...
 *
 * <p>탐색용 속성({@code categoryCode}·{@code description}·{@code imageUrl}·{@code brand})은
 * 쇼핑몰 화면을 위해 더한 것이고, 가격·재고의 권위에는 관여하지 않는다. 쓰기 표면은 없고
 * 시드와 마이그레이션으로만 채운다 — 조회는 {@link CatalogQueryService}가 담당한다.
 */
@Entity
@Table(name = "products")
@Getter
...
public class Product {
    ...
    private Product(long productId, String name, long price, String categoryCode, String description,
                    String imageUrl, String brand, boolean featured, Instant createdAt) {
        if (price < 0) {
    ...
    public static Product of(long productId, String name, long price) {
        return new Product(productId, name, price, null, null, null, null, false, null);
    }

    /** 카탈로그 시드·조회가 쓰는 전체 팩토리. */
    public static Product of(long productId, String name, long price, String categoryCode,
        ...
        return new Product(productId, name, price, categoryCode, description, imageUrl, brand,
                featured, createdAt);
    }
}
