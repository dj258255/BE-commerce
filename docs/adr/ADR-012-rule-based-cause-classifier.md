...
원인 목록이 **빠진 게 없는지**는 스스로 세어서는 알 수 없다. 업계에서 반복되는
...
**규칙 제안은 붙이지 않았다.** 중복은 매칭 **전에** 걸러지므로 남은 불일치만 보고는
...
찾은 것은 데이터가 아니라 **분류표**였고, 그게 더 쓸모 있었다 —
표본이 없어도 **목록에 구멍이 있는지**는 확인할 수 있다.
