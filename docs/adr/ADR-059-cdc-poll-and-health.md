...
- 커넥터가 늘어 Connect CPU 가 의미 있게 오를 때 → poll 을 커넥터별로 다시 고른다. #360 에서 둘(카탈로그 · 활동)을 같이 띄워 쟀다: 100 대 500 차이가 조용할 때 +0.4%p · 변경 중 +1.0%p 라 둘 다 100 을 유지한다
...
## 커넥터 둘을 같이 띄운 비용 (#360, 2026-09-26)

일회용 CDC 스택에서 저장소의 두 커넥터(`catalog-cdc` · `user-activity`)를 같이 띄워 #252 와 같은 방법(조용한 60초 · 변경 60초, `docker stats` 2초 표본)으로 쟀다.
변경 구간은 상품 가격 초당 50건에 활동 초당 50건을 더했다. 예측과 기준은 이슈에 측정 전에 적었다.

| 조건 | Connect CPU 조용 | Connect CPU 변경 중 | 카탈로그 지연 p95 |
|---|---:|---:|---:|
| 카탈로그만 · poll 100 | 1.1% | 4.6% | 97ms |
| 카탈로그만 · poll 100 · 활동 부하(DB 부하는 아래와 같음) | 1.0% | 4.4% | 97ms |
| **둘 · poll 100** | 1.2% | 5.5% | 97ms |
| 둘 · poll 500 | 0.8% | 4.5% | 478ms |

- **둘 다 100 을 유지한다.** poll 100 대 500 차이가 조용할 때 +0.4%p, 변경 중 +1.0%p 로 기준(5%p)에 한참 못 미친다
- 커넥터를 하나 더해도 변경 중 +0.9%p 였다. 캡처하지 않는 표의 binlog 를 거르는 비용은 보이지 않았다(활동 부하를 더해도 카탈로그만 띄운 값이 같았다)
- 예측(조용할 때 CPU 가 커넥터 수만큼 는다)은 틀렸다. 이번 절대값이 #252(조용할 때 3.0%)보다 낮은 이유는 재지 않았다
- 원자료 [`performance/raw/20260926-a059-360/`](../performance/raw/20260926-a059-360/)
