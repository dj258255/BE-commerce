...
> 결제 화면의 `UNKNOWN`은 오류를 숨긴 값이 아니다. 서버가 PG 승인 여부를 확인하지 못한
> 상태를 사용자에게 드러내고, 주문 조회 폴링으로 최종 상태를 확인하게 하는 상태다. 실제
> PG 결제창·실제 웹훅 수신은 외부 계약 환경이 필요하므로 이 문서의 화면 검증은 테스트 PG와
> 통합 테스트 범위로 한정한다.
...
```
commerce/src/main/resources/static/
├── index.html          홈
├── category.html       카테고리
├── search.html         검색 결과
├── product.html        상품 상세
...
| 카테고리 | `category.html?code=&sort=&page=` | `GET /categories?tree=true`, `/products?category=&sort=&page=` |
...
- **컴포넌트**: 헤더(검색·장바구니 배지·계정), 상품 카드, 배지, 페이지네이션, 빈 상태, 토스트,
  스켈레톤, 개발자 로그 드로어.
- **상품 비주얼**: 상품 사진은 H&M 원본에서 받은 **실제 이미지**다(커버리지 **19.4%** — 아직 전부는 아니다,
  `personalization/docs/runs/hm-images.md`). 이미지가 없는 상품은 상품 id로 만든 그라디언트 타일로
  폴백해 레이아웃이 유지된다.
